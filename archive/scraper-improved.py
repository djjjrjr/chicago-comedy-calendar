...
- Uses multiple Do312 endpoints (today, tomorrow, week)
...
def scrape_page(page, url: str, page_name: str) -> List[Dict]:
    """Scrape a single Do312 page and return events"""
    events = []
    ...
    try:
        ...
        # Try to wait for event cards
        try:
            page.wait_for_selector('.ds-listing.event-card', timeout=10000)
        except PlaywrightTimeout:
            print(f"  No events found on this page\n")
            return events
        ...
        print(f"  Found {len(events_data)} events on this page")
        return events_data

    except PlaywrightTimeout as e:
        print(f"  ⚠️  Timeout loading page: {e}\n")
        return events
...
def scrape_do312_comedy_events(page) -> List[Dict]:
    """Scrape all comedy events from Do312 using multiple endpoints"""
    all_events = []
    ...
    print(f"Using multiple endpoints: today, tomorrow, week, and main page\n")

    # Define the pages to scrape
    pages_to_scrape = [
        (f"{base_url}/today", "Today's Comedy Events"),
        (f"{base_url}/tomorrow", "Tomorrow's Comedy Events"),
        (f"{base_url}/week", "This Week's Comedy Events"),
        (f"{base_url}", "All Upcoming Comedy Events"),
    ]

    total_raw_events = 0

    for url, page_name in pages_to_scrape:
        events_data = scrape_page(page, url, page_name)
        total_raw_events += len(events_data)
        ...
        print(f"  Added {new_count} new unique events (after deduplication)")
        print(f"  Total unique events so far: {len(all_events)}\n")

    print(f"\n{'='*60}")
    print(f"SCRAPING COMPLETE")
    print(f"{'='*60}")
    print(f"Total raw events found: {total_raw_events}")
    print(f"Total unique events: {len(all_events)}")
    print(f"Duplicates removed: {total_raw_events - len(all_events)}\n")
...
def scrape_all_events() -> List[Dict]:
    ...
    print("Scraping comedy events from Do312.com\n")
