...
if [ ! -f "the-stand-shows.json" ]; then
    echo "⚠️  Warning: the-stand-shows.json not found"
    MISSING_FILES=$((MISSING_FILES + 1))
fi

if [ $MISSING_FILES -eq 5 ]; then
...
def normalize_venue_name(venue):
    ...
    # Comedy Cellar has multiple distinct rooms that host simultaneous shows.
    return venue
...
def main():
    ...
    sources = [
        ...
        ('The Stand', 'the-stand-shows.json'),
        ('UCB Theatre', 'ucb-ny-shows.json')
    ]
